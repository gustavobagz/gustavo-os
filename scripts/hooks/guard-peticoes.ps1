...
#
# IMPORTANTE - este arquivo e deliberadamente 100% ASCII. O Windows PowerShell 5.1
# le .ps1 sem BOM como ANSI, o que corrompe literais acentuados no fonte; por isso
# todo caractere acentuado dos regex vai como escape \uXXXX. Nao introduza acentos
# aqui: o hook passa a falhar em silencio.
#
...
$regras = @(
    ...
    @{ P = '(?i)(?<![\w-])(?:r[eé]us?|r[eé]s|ré)(?![\w-])'; M = 'nomenclatura "reu/re" - usar "requerida"' }
)
...
for ($i = 0; $i -lt $linhas.Count; $i++) {
    ...
    $naLinha = New-Object System.Collections.ArrayList
    foreach ($r in $regras) {
        $m = [regex]::Match($linha, $r.P)
        if ($m.Success) { [void]$naLinha.Add($r.M) }
    }
    if ($naLinha.Count -eq 0) { continue }

    $trecho = $linha.Trim()
    if ($trecho.Length -gt 70) { $trecho = $trecho.Substring(0, 70) + '...' }
    [void]$achados.Add(("  linha {0}: {1}`n     > {2}" -f ($i + 1), ($naLinha -join '; '), $trecho))
}

if ($achados.Count -eq 0) { exit 0 }

$nome  = Split-Path -Leaf $caminho
$lista = ($achados | Select-Object -First 15) -join "`n"
$resto = if ($achados.Count -gt 15) { "`n  (+{0} outra(s) linha(s))" -f ($achados.Count - 15) } else { '' }
